        ...
        <tbody>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td class='action-buttons'>
                        ...
                        <button class='btn btn-success btn-sm' onclick='showConfirmAccModal(\"$idsm\")'>Acc</button>
                        </td>";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmAccModalLabel">Konfirmasi</h5>
...
<script>
    ...
    document.getElementById('confirmAccButton').addEventListener('click', function() {
        if (currentAccId) {
            updateStatus("Acc", currentAccId);
            var modal = bootstrap.Modal.getInstance(document.getElementById('confirmAccModal'));
            modal.hide();
        }
    });

    function updateStatus(status, idsm) {
        // Send AJAX request to update status
        $.ajax({
            url: 'update_status.php', // Point to your update status script
            method: 'POST',
            data: { status: status, idsm: idsm },
